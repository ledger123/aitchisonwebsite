...
if ( in_array($_SESSION['portal_School'], ['JSD']) ) {
	$accountRepogcycle = new AccountRepository($dc);    
	$studentgroups = $accountRepogcycle->getStudentGroupCycle($_SESSION['portal_SchoolNo']);
	if($studentgroups->rowsCount() > 0){
		$studentgroup=$studentgroups->Rows[0];
		$groupcycle=$studentgroup['cycle'];
		if (in_array($_SESSION['portal_CurrentClass'], ['K1', 'K2', 'K3'])){
			if($groupcycle=="A"){
		...
		if (in_array($_SESSION['portal_CurrentClass'], ['K4', 'K5'])){
			if($groupcycle=="A"){
				$msg_student="Your classes are scheduled on Monday, Wednesday and Friday.";
			}else{
				$msg_student="Your classes are scheduled on Tuesday, Thursday and Saturday.";
		...
		if (in_array($_SESSION['portal_CurrentClass'], ['K6'])){
			if($groupcycle=="A"){
				$msg_student="Your classes are scheduled on Monday, Wednesday and Friday.";
			}else{
				$msg_student="Your classes are scheduled on Tuesday, Thursday and Saturday.";
