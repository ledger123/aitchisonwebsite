...
$banner_text="";
$default_open=0;
if(intval($_GET['id'])==330 || intval($_GET['id'])==417 || intval($_GET['id'])==423 || intval($_GET['id'])==418){
	$default_open=5;
	if((count($_SESSION['stdportal']) > 0) || (count($_SESSION['boarder']) > 0) || (count($_SESSION['stfportal']) > 0) || (count($_SESSION['parents']) > 0)){
		echo "<!--Akhtar Rafiq-->";
		$default_open=intval($default_open)+1;	
	}
}
if(intval($_GET['id'])==576){
	$newslink="./portal/principal-letter.php?id=575";
	header("location: $newslink");
}
