$(document).ready(function() {
	...
	*/
	...
	if( $('#TxtNewsContents').length > 0 )
	CKEDITOR.replace( 'TxtNewsContents', {
		extraAllowedContent: '*[id,role,img,align,video,source](*){*};a[href,rel,target,download,style];img[src,width,height,alt,title];video[width,autoplay,controls];source[src,type];ol;li',
		toolbar: cktoolbar
	});
	
	if( $('#TxtNewsContents2').length > 0 )
	CKEDITOR.replace( 'TxtNewsContents2', { 
		extraAllowedContent: '*[id,role,img,align,video,source](*){*};a[href,rel,target,download,style];img[src,width,height,alt,title];video[width,autoplay,controls];source[src,type];ol;li',
		toolbar: cktoolbar } );
	
	
	if( $('#TxtNewsSubheading2').length > 0 )
	CKEDITOR.replace( 'TxtNewsSubheading2', {
		extraAllowedContent: '*[id,role,img,align,video,source](*){*};a[href,rel,target,download,style];img[src,width,height,alt,title];video[width,autoplay,controls];source[src,type];ol;li',
		toolbar: cktoolbar2
	});
...
function onViewNews(id)
{
	...
	window.open("../../newsandevents-testlink.php?id="+id+"&mode=view",'_blank');
}
