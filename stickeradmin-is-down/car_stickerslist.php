...
// Build SQL
$sSql = "SELECT c.id, request_date, requestor_name, relation_with_student, requestor_email, sticker_num, car_num1, car_num2, car_pool_period, car_pool_amount, school_num1, school_num2, school_num3, school_num4, school_num5, driver1_name, driver1_relation, driver2_name, driver2_relation, driver3_name, driver3_relation, driver4_name, driver4_relation, driver5_name, driver5_relation, issued, received, user, permission, sticker_for, std_name1, std_class1, std_name2, std_class2, std_name3, std_class3, std_name4, std_class4, std_name5, std_class5, status1, status2, status3, status4, status5
, s1.school_name school_name1
, s2.school_name school_name2
, s3.school_name school_name3
, s4.school_name school_name4
FROM car_stickers c
LEFT JOIN ac_students s1 ON s1.sch_num = c.school_num1
LEFT JOIN ac_students s2 ON s2.sch_num = c.school_num2
LEFT JOIN ac_students s3 ON s3.sch_num = c.school_num3
LEFT JOIN ac_students s4 ON s4.sch_num = c.school_num4
";
...
//guru_query('update ,$conn) or die("Error: " . guru_error() . '<br>SQL: ' . $updateSQL);
...
while (($row = @guru_fetch_array($rs)) && ($nRecCount < $nStopRec)) {
	...
	if ($nRecCount >= $nStartRec) {
		...
		$x_std_class1 = $row["std_class1"] . " - " . $row["school_name1"];
		$x_school_num2 = $row["school_num2"];
		$x_std_name2 = $row["std_name2"];
		$x_std_class2 = $row["std_class2"] . " - " . $row["school_name2"];
		$x_school_num3 = $row["school_num3"];
		$x_std_name3 = $row["std_name3"];
		$x_std_class3 = $row["std_class3"] . " - " . $row["school_name3"];
		$x_school_num4 = $row["school_num4"];
		$x_std_name4 = $row["std_name4"];
		$x_std_class4 = $row["std_class4"] . " - " . $row["school_name4"];
...
<?php }else{ ?>
<?php echo "$x_school_num1 ($status1)"; ?>
<?php } ?>
</span></td>
		<!-- std_name1 -->
		<td><span class="guru">
...
<?php }else{ ?>
<?php echo "$x_school_num2 ($status2)"; ?>
<?php } ?>
</span></td>
		<!-- std_name2 -->
		<td><span class="guru">
...
<?php }else{ ?>
<?php echo "$x_school_num3 ($status3)"; ?>
<?php } ?>
</span></td>
		<!-- std_name3 -->
		<td><span class="guru">
...
<?php }else{ ?>
<?php echo "$x_school_num4 ($status4)"; ?>
<?php } ?>
</span></td>
		<!-- std_name4 -->
		<td><span class="guru">
