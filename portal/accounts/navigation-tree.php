...
elseif (isset($_POST['cmdSearchByParent'])) {
	$sectionNew = false;
	$sectionView = true;
...
elseif ( $id == 0 && $TxtEditId > 0 )
{
	...
	$TxtLevel	 	    = $parentId == $TxtEditId			? 1							: 2;
    ...
    $TxtAddress2   		= isset($_POST['TxtAddress2'])		? $_POST['TxtAddress2']	    : "";
	...
	{
		if (true || !$accountsRepo->IsNodeExist( $TxtEditId, $TxtNode ) )
		{
			//								($id, $parentId, $level, $title, $node, $address, $icon, $displayOrder, $show, $active)
			$response = $accountsRepo->UpdateNode($TxtEditId, $parentId, $TxtLevel, $TxtTitle, $TxtNode, $TxtAddress, $TxtAddress2, $TxtIcon, $TxtDisplayOrder, $optShow, $optActive);
...
elseif ( isset($_POST['cmdSubmit']) && $id == 0 && $TxtEditId == 0 )
{
	...
	if( strlen ($TxtNode) > 3 )
	{
		if ( !$accountsRepo->IsNodeExist(0, $TxtNode) )
		{
			...
			$response = $accountsRepo->SaveNode($parentId, $TxtLevel, $TxtTitle, $TxtNode, $TxtAddress, $TxtAddress, $TxtIcon, $TxtDisplayOrder, $optShow, $optActive);
...
$data = $accountsRepo->GetNavigationTree(0, $offset, $recordsPerPage, $parentId);
...
<?php $title = "Navigation Tree"; include($path."includes.php"); ?>
<style>
label.error {
	color: red;
	font-weight: 500;
					...
					<div class="x_panel">
					  <div class="x_content">
						  ...
						  <div class="">
							  ...
							  <div class="" style="">
								<br>
								  <form id="formNew" action="navigationtree" method="post" data-parsley-validate="" class="form-horizontal form-label-left" novalidate="">
									...
									<?php if($response == 0) { ?>
									 ...
									 <input type="hidden" id="Txtg" name="Txtg" value="<?php echo $page; ?>" />
									  <input type="hidden" id="TxtLevel" name="TxtLevel" value="<?php echo $TxtLevel; ?>" />
                                     ...
                                     <div class="form-group">
                                         <label for="TxtAddress2" class="control-label col-md-3 col-sm-3 col-xs-12">Slug</label>
                                         <div class="col-md-6 col-sm-6 col-xs-12">
                                             <input id="TxtAddress2" class="form-control col-md-7 col-xs-12" type="text" name="TxtAddress2" data-parsley-id="9" value="<?php echo htmlentities(stripslashes(utf8_decode($TxtAddress2))); ?>">
                                         </div>
                                     </div>
								...
								<section id="secView">
									<div class="row">
                                        ...
                                        <div class="col-lg-1 col-md-1 col-sm-1" style="text-align: right;">
                                            <button type="submit" id="cmdSearchByParent" name="cmdSearchByParent" class="btn btn-primary">Search</button>
                                        </div>
                                    </div>
