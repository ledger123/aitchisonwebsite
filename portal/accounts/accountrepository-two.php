...
class AccountRepository
{
    ...
    public function GetNavigationTree($id, $offset, $recordsPerPage)
    {
        $id = (int)$id;
        ...
        $qry =
            "SELECT t1.`Id`, t2.`Id` ParentId, t2.`Node` Parent, t1.`Level`, t1.`Title`, t1.`Node`, t1.`Address`, t1.`Icon`, t1.`DisplayOrder`,
    ...
    public function SaveNode($parentId, $level, $title, $node, $address, $icon, $displayOrder, $show, $active)
    {
        ...
        $qry = "INSERT INTO portal_navigation_tree (ParentId, `Level`, Title, Node, Address, Icon, DisplayOrder, `Show`, Active)
		VALUES (?,?,?,?,?,?,?,?,?)";
    ...
    public function UpdateNode($id, $parentId, $level, $title, $node, $address, $icon, $displayOrder, $show, $active)
    {
        $id = (int)$id;
        $parentId = (int)$parentId;
        $level	  = (int)$level;
        ...
        $qry =
			...
			SET ParentId = ?, `Level` = ?, Title = ?, Node = ?, Address = ?, Icon = ?, DisplayOrder = ?, `Show` = ?, Active = ?
			WHERE Id = ?";
