...
class MenuItem
{
	...
	function __construct($title = '', $url = '', $urlTarget = '', $parent, $icon)
	{
		$this->Items = array();
	...
	function Get($last)
	{
		...
		$target = "";
		if ($this->UrlTarget != "") {
			$target = "target='". $this->UrlTarget ."'";
		}
		
		$nl = "\n";
		if (trim($this->Url) == "#")
			$html = "<li " . $class ."><a $style href='".$this->Url."' ". $target .">".$icon." ".$this->Title.$downArraw."</a>";
			
		else if ( strlen( trim($this->Url) ) > 0 )
			$html = "<li " . $class ."><a $style href='".$this->Url."' ". $target .">".$icon." ".$this->Title."</a>";
		...
		if ( $counts > 0 )
		{
			$html .= $nl."<ul class='nav child_menu' style='display=block;'>".$nl;
			$index = 1; 
