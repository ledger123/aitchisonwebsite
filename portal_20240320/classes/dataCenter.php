...
class DataCenter {
	...
	function Connect2()
	{
		$this->conn = new mysqli(App::$server,App::$userName,App::$password, App::$databaseName2); // mysqli_connect(App::$server,App::$userName,App::$password);
		
		$this->connOpen = true;
		
		$GLOBALS['conn2'] = $this->conn;
	}
	...
	public function ExecuteParamQuery( $query, $params, $isTable = false )
	{
		try
		{
			...
			if ( $isTable === true )
			{
				...
				$dTable->SetDataSourceAsArray($result);
				return $dTable;
			}
	...
	public function ExecuteQuery($query, $isTable = false)
	{
		try
		{
			...
			$result = mysqli_query($this->conn, $query) or die("Data not found."); // die("Query Could not Execute Due to " . mysqli_error($this->conn));
	...
	public function ExecuteNonQuery1($query)
	{
		try
		{
			...
			mysqli_query($this->conn, $query, MYSQLI_USE_RESULT) or die("Data not found."); //  die("Query Could not Execute Due to " . mysqli_error($this->conn));
	...
	public function ExecuteMultiQuery( $query ) {
		...
		//mysqli_multi_query( $this->conn, $query ) or die("Query Could not Execute Due to " . mysqli_error($this->conn));
