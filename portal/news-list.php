...
<?php $title = "Latest News"; include($path."includes.php"); ?>
<style>
#defaultIcon {
  position: relative;
}
...
.card-body1{
	...
	.card-title{
		...
		overflow:  hidden;
		/*
		span.blogwidget_title {font-family: "myriad-pro", "Open Sans", sans-serif;font-size: 20px;color: #000;text-transform: uppercase;padding: 20px 10px 0 10px !important;display: block;min-height: 120px;}
		*/
	}
...
.btn {
  background-color: #002663;/*DodgerBlue;*/
  border: none;
					  ...
					  <div class="page-title">
                    	...
                    	<br />
                  		<div class="news-item">
							...
							$qry_news="SELECT  DISTINCT `Id` , `NewsFor`, `NewsType`, `Heading` ,  `Subheading2` ,  `NewsIcon`, TemplateId, CreateDate FROM  `n_news_news` WHERE `Active`=1 AND Archived = 0  and `Id` IN ( ".$query_details." ) and TemplateId != 4 and `NewsType` !='Notice' ORDER BY  `n_news_news`.`CreateDate` DESC Limit 0, 12";
							/*echo "<br><br>";
							echo $qry_news;
							echo "<br><br>";	*/
...
//							print_r($dTable);
								...
								<a href="<?php echo $newslink; ?>" target="_blank">
									<div class="card">
										...
										<div class="card-body">
											...
											<h4 class="card-title"><?php echo $purifier->purify(stripslashes($row->Heading)); ?></h4>
											<?php /*?><p class="card-text"><?php echo str_replace("</p>","", str_replace("<p>","",$row->Subheading2)); ?>&nbsp;</p><?php */?>
											</div>	
											<div class="btn btn_width">Read More</div>
										</div>
									</div>
								</a>
							</div>
					...
					<div  align="right" style="padding-right: 1.2em;"><button class="readmore"><a href="news-archive.php"><span style="color: white !important;">Archived News</span></a></button></div>
							<p>&nbsp;</p>		
                  </div>
				 <br>
<br>
