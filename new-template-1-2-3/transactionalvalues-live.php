<?php
	...
	/***************STARTED FOR NEW AMMENDMENTS AT 2021-04-06 ******************/
	...
	/***************ENDED FOR NEW AMMENDMENTS AT 2021-04-06 *******************/	
	/*$banner_image_url="img/pageimages/misc.jpg";
	$banner_text="";
	$default_open=-1; */
	if(!empty($_POST) && $signature_verify=="True"){
		$pagename="transactionalvalues-live.php";
		...
		$fields_arr = $params; //explode(",", $fieldslength);
		//print_r($fields_arr);
		$insert_array=array();
		$params_db = array();
		$insert_keys="";
		...
		for($i=0 ; $i < count($params) ; $i++){
			...
			if(isset($_POST[$fieldvalue]) && strlen($fieldvalue) > 3){
				...
				if(strlen($insert_keys) > 1){
					$insert_keys=", ".$fieldvalue;
					$insert_value=", ?";
				}else{
					$insert_keys=$fieldvalue;
					$insert_value="?";
				}
				$params_db[] = array("datatype"=>"s", "value"=>$_POST[$fieldvalue]);
			}
		}
		if(isset($_POST['auth_cv_result'])){
			$insert_keys=", cvn_desc";
			$insert_value=", ?";
			$params_db[] = array("datatype"=>"s", "value"=>$cvn_array_message[trim($_POST['auth_cv_result'])]);
			//$insert_array['cvn_desc']=$cvn_array_message[trim($_POST['auth_cv_result'])];
		}else{
			$insert_keys=", cvn_desc";
			$insert_value=", ?";
			$params_db[] = array("datatype"=>"s", "value"=>$cvn_array_message['130']);
			//$insert_array['cvn_desc']=$cvn_array_message['130'];				
		}		
		...
		$hbl_transaction_qry="insert into `hbl_transaction` (".$insert_keys.") values (".$insert_value.")";
		//$db->insertRecordNoAuto("hbl_transaction" , $insert_array);
		$lastInsertId = $dc->ExecuteNonQuery($hbl_transaction_qry, $params_db, true);
		//$db->executeQuery($hbl_donations_query);
		//ExecuteQuery($query, $isTable = false)
