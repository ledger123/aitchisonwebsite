...
class WeeklyActivityGlimpses
{
	...
	public function GetWeeklyGlimpses74($pageId = 0) {
		$pagesize=12;
		$pageid=$pageId;
		$recordStart=$pageid*$pagesize;
		/*if($pageId == 0){			
			$pagesize=(int)$pagesize+1;
			$recordStart=(int)$pagesize*$pagesize;
		}else{
			$recordStart=(int)$pagesize*$pagesize+1;
		}*/
		$qry="SELECT * FROM `n_news_news` INNER JOIN `n_news_publish_locations` ON `n_news_news`.`Id`=`n_news_publish_locations`.`NewsId` WHERE `TemplateId` in (2, 5) AND `LocationCode`=74 and `Active`=1 ORDER BY `n_news_news`.`Date` DESC, `n_news_news`.`Id` DESC ";
		$qry.="Limit ".$recordStart.", ".$pagesize;
		//echo $qry;
		$dTable = $this->dc->ExecuteQuery($qry, true);
		return $dTable;
		// Limit 0, 15
	}
	public function GetWeeklyGlimpses($pageId = 0) {
		$pagesize=12;
		$pageid=$pageId;
		$recordStart=$pageid*$pagesize;
		/*if($pageId == 0){			
		...
		$qry="SELECT * FROM `n_news_news` INNER JOIN `n_news_publish_locations` ON `n_news_news`.`Id`=`n_news_publish_locations`.`NewsId` WHERE `TemplateId` in (2, 5) AND `LocationCode`=3 and `Active`=1 ORDER BY `n_news_news`.`Date` DESC, `n_news_news`.`Id` DESC ";
		$qry.="Limit ".$recordStart.", ".$pagesize;
		//echo $qry;
		$dTable = $this->dc->ExecuteQuery($qry, true);
		return $dTable;
...
//		echo "</pre>";
	...
	public function SearchNews($searchText, $offset = 0, $recordsPerPage = 1, $active = -1, $userId)
	{
		...
		$qry = "SELECT DISTINCT n.Id, TemplateId, t.Name Template, Date, REPLACE(Heading, '\'\'', '\'') Heading, REPLACE(Subheading, '\'\'', '\'') Subheading, NewsType, ShowHeading, IFNULL(n_users2.Title, '') ApprovedBy, NewsIcon,
				CASE WHEN n.Active = 1 THEN 'Yes' ELSE 'No' END Active, n_users1.Title UserTitle, CreateDate, OpenAccess " . $qry;
    ...
    function GetPublishedNewsForNewsroom( $locationIds, $month, $year, $offset = 0, $recordsPerPage = 0, $excludeId = 0 )
    {
        ...
        $qry =
            "SELECT DISTINCT n.Id NewsId, n.TemplateId as TemplateId, DATE_FORMAT(n.Date, '%e %M, %Y') Date, n.ShowDate, n.Date nDate, 
			REPLACE(n.Heading, '\'\'', '\'') Heading, REPLACE(n.Subheading, '\'\'', '\'') Subheading, REPLACE(n.Subheading2, '\'\'', '\'') Subheading2, 
			n.NewsIcon, n.NewsIconLink, n.NewsType, '' Content1, '' Downloadable, IFNULL( CONCAT( LEFT( EXTRACTVALUE(REPLACE(c.Content1, '&quot;', '\"'), '//p'), 265), '...'), '...') Content4, `NewsFor`
			FROM n_news_news n INNER JOIN
				n_news_publish_locations pl ON (n.Id = pl.NewsId) INNER JOIN
                n_news_templates t ON ( t.Id = n.TemplateId ) LEFT JOIN 
                n_news_contents c ON c.NewsId = n.Id
