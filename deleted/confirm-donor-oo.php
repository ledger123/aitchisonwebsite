<?php
	...
	if(count($_POST) > 0){
		if($_SERVER['HTTP_REFERER']=="https://aitchison.edu.pk/d-one.php"){			
		...
		echo "AAA";
		$ac_aff=implode(", ", $_POST['affiliation']);
		$fname=$_POST['fname'];
		$lname=$_POST['lname'];
		$email=$_POST['email'];
		$phone=$_POST['phone'];
		...
		//print_r($_POST);
		//$=$_POST[''];
		$stmt = $con->prepare("INSERT INTO donor_information (amount, currency, proj_name, anonymous, ac_aff, fname, lname, email, phone, address, token, sess_id, ddate, timestamp, country, state, city, postalcode) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		if($stmt){
			$stmt->bind_param("isssssssssississss", $amount, $currency, $proj_name, $anonymous, $ac_aff, $fname, $lname, $email, $phone, $address, $token, $sess_id, $ddate, $timestamp, $country, $state, $city, $postalcode);
			$stmt->execute();	
